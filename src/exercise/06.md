...
Props, props, and more props. React has a robust number of events that can be
set which are actually 'synthetic events'. To use these events they are declared
as props within JSX. The events work practically the same as regular events, but
they are specific to React.

Extra credit touched on state, hooks, and controlled form inputs. State between
JavaScript and the page can be persistent which can help with input validation.

Controlled form inputs enable you to use React to 'control' the state of an
input field. Combined with an onChange handler interesting functionality is
possible like auto-correcting a user input to be lowercase.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=06%3A%20Forms&em=marcacard%40gmail.com).
